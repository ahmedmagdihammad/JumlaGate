            ...
            search: {
                ...
                afterTemplate: function (row, data, index) {
                    row.find('.delete-item').on('click', function () {
                        swal.fire({
                            text: locator.__("Are you sure to delete this item?"),
                            ...
                            if (result.value) {
                                ...
                                $.ajax({
                                    ...
                                    swal.fire({
                                        text: locator.__("Deleted Successfully !"),
                                        confirmButtonText: locator.__("Ok"),
                                        icon: "success",
                                        confirmButtonClass: "btn font-weight-bold btn-primary",
                                    });
            ...
            columns: [
                {
                    ...
                    template: function(data) {
                        ...
                        var output = '';
                        if (data.logo != null) {
                            var user_img = 'background-image:url(../factory_logos/' + data.logo + ')';
                            output = '<div class="d-flex align-items-center">\
								<div class="symbol symbol-40 flex-shrink-0">\
									<div class="symbol-label" style="' + user_img + '"></div>\
								</div>\
								<div class="ml-2">\
									<div class="text-dark-75 font-weight-bold line-height-sm">' + data.name_en + '</div>\
                            ...
                            output = '<div class="d-flex align-items-center">\
								<div class="symbol symbol-40 symbol-'+state+' flex-shrink-0">\
									<div class="symbol-label">' + data.name_en.substring(0, 1) + '</div>\
								</div>\
								<div class="ml-2">\
									<div class="text-dark-75 font-weight-bold line-height-sm">' + data.name_en + '</div>\
								</div>\
                    ...
                    template: function(row) {
                        ...
                        <div class="dropdown dropdown-inline">\
                            <a href="/dashboard/companies/' + row.id  + '" class="btn btn-sm btn-clean btn-icon mr-2" title="Show">\
                             \<i class="flaticon-eye"></i>\
                            </a>\
                        </div>\
                        <a href="/dashboard/companies/'+ row.id +'/edit" class="btn btn-sm btn-clean btn-icon mr-2" title="Edit details">\
